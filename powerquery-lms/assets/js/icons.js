// Mapa de iconos SVG (subconjunto de lucide, embebidos a mano — sin CDN).
// Unión de los ICON_MAP usados en los 5 módulos originales (48 iconos, sin conflictos
// de nombre). Cargar antes de engine.js.
const ICON_MAP = {
...
// Reemplaza cada <i data-lucide="nombre" class="..."> por un <svg> real, 100% local.
function safeIcons() {
  try {
    document.querySelectorAll('[data-lucide]').forEach(el => {
      const name = el.getAttribute('data-lucide');
      const inner = ICON_MAP[name];
      if (!inner) return;
  ...
  } catch (e) { console.warn('render de iconos falló:', e); }
}
